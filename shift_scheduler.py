...
def generate_schedule(guides, tours, max_per_guide=None):
    ...
    1. Pre-compute volunteer lists for all special tours.
    2. Build a list of "assignments to make":
         - Each unique weekly slot  (slot_key) counts as one assignment worth n_weeks tours.
         - Each special tour        counts as one assignment worth 1 tour.
    3. Sort all assignments by how many eligible candidates they have (fewest first —
       most constrained gets priority), breaking ties by highest weight (biggest slots
       first so they anchor the schedule).
    4. Work through the sorted list, always picking the eligible guide(s) with the
       lowest current normalised load (count / total_eligible_slots) so that guides
       with fewer options are not starved.
    """
    avail      = {g["name"]: set(s.lower() for s in g["slots"]) for g in guides}
    count      = {g["name"]: 0 for g in guides}   # total tours assigned so far
    schedule   = {}
    volunteers = {}
    ...
    # Each entry: (kind, key_or_id, eligible_names, min_g, max_g, weight)
    work = []
    ...
    # Sort: fewest eligible first (most constrained), then heaviest weight first
    work.sort(key=lambda w: (len(w[2]), -w[5]))

    # ── Assign ────────────────────────────────────────────────────────────────
    slot_assigned = {}   # slot_key -> list of names

    # Pre-compute how many slots each guide is eligible for (across all work items)
    # Used to normalise load: guides with fewer options get priority.
    ...
    def pick(eligible, max_g, weight):
        """Return up to max_g names sorted by fairness score (lowest load first)."""
        if max_per_guide:
            eligible = [n for n in eligible if count.get(n, 0) + weight <= max_per_guide]
        # Normalised load = count / max(1, total eligible weight)
        # Guides with fewer eligible slots get a nudge toward assignment (lower score)
        def score(n):
            total_elig = max(1, eligibility_count[n])
            return (count.get(n, 0) / total_elig, count.get(n, 0), n)
        return sorted(eligible, key=score)[:max_g]
    ...
    for kind, key, eligible, min_g, max_g, weight in work:
        ...
        if kind == "weekly":
            ...
            for n in assigned:
                count[n] = count.get(n, 0) + weight   # weight = n_weeks
        else:
  ...
  <div class="pane active" id="pane-upload">
    ...
    <div class="two-up">
      ...
      <div class="section">
        ...
        <p class="msg-err" id="err-guides"></p><p class="msg-ok" id="ok-guides"></p>
      </div>
    </div>
    <div class="field-row">
...
<script>
let tours=[],guides=[],scheduleResult={},countResult={},volunteersResult={};
...
function handleGuideFile(file){readFile(file,text=>{fetch('/parse_guides',{method:'POST',headers:{'Content-Type':'text/plain'},body:text}).then(r=>r.json()).then(data=>{document.getElementById('err-guides').textContent=data.error||'';document.getElementById('ok-guides').textContent=data.error?'':`${data.guides.length} guides &mdash; ${data.guides.filter(g=>g.slots.length>0).length} with availability.`;if(!data.error){guides=data.guides;document.getElementById('dz-guides').classList.add('loaded');document.getElementById('dz-guides-label').textContent=file.name;document.getElementById('dz-guides-hint').textContent='Loaded';}});});}
