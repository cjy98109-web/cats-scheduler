...
def parse_guide_csv(raw_text):
    ...
    # Deduplicate by email, keep latest row
    seen = {}
    for row in rows[1:]:
...
def generate_schedule(guides, tours, max_per_guide=None):
    ...
    # Unified work list: (kind, key, eligible, min_g, max_g, weight)
    work = []
    ...
    # Shuffle then sort: most constrained first, heaviest weight first
    random.shuffle(work)
...
var scheduleResult={}, countResult={}, volunteersResult={};
...
function generateSchedule(){
  ...
  .then(function(data){
    ...
    scheduleResult=data.schedule;countResult=data.count;volunteersResult=data.volunteers;
    renderSchedule();
    showPane('schedule');
  });
}
