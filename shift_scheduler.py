...
def parse_oneoff_csv(raw_text):
    """
    Parse a spreadsheet of one-off tours in the format:
      Date | Time | Event | # TGs | Notes
    Accepts tab-separated or CSV. Handles:
      - Date formats: "June 23", "7/1", "July 20"
      - Time formats: "9:45 AM", "11am", "11am-12pm" (takes start), "AM" (-> 9:00 AM)
      - Guide counts: "4", "10-12" (min-max), blank (-> 1-2)
      - Strips URLs from event names
    Returns (tours list, error or None).
    """
    import re
    ...
    def parse_date(raw):
        ...
        if m:
            ...
            try:
                d = _date(2026, mo, day)
                return f"{DAY_ABBR[d.weekday()]} {MON_ABBR[mo]} {day}", DAY_FULL[d.weekday()]
            except: pass
        # "June 23" or "July 20"
        m = re.match(r'^([A-Za-z]+)\s+(\d{1,2})$', raw)
        if m:
            mo_n = MONTH_MAP.get(m.group(1).lower())
            day  = int(m.group(2))
            if mo_n:
                try:
                    d = _date(2026, mo_n, day)
                    return f"{DAY_ABBR[d.weekday()]} {MON_ABBR[mo_n]} {day}", DAY_FULL[d.weekday()]
                except: pass
        return raw, ''

    def parse_time(raw):
        raw = raw.strip()
        if raw.upper() in ('AM',): return '9:00 AM'
        if raw.upper() in ('PM',): return '12:00 PM'
        # "11am-12pm" or "9:00-10:00 AM" — take start portion only
    ...
    def clean_name(raw):
        raw = re.sub(r'https?://\S+', '', raw).strip().strip(',').strip()
        return raw
    ...
    rows = list(reader)
    if not rows: return [], "Empty file."

    # Skip header row
    start = 0
    if rows[0] and rows[0][0].strip().lower() in ('date', 'dates'):
        start = 1
    ...
    for row in rows[start:]:
        ...
        date_str, day_name = parse_date(date_raw)
        time_str = parse_time(time_raw)
        name     = clean_name(name_raw)
        min_g, max_g = parse_guides(tg_raw)
        ...
        tours.append({
            ...
            "max_guides": max_g,
            "slot":       slot,
            "sort_key":   "1" + date_str + time_str,
            "is_oneoff":  True,
            "slot_key":   "",
        })

    if not tours:
        return [], "No valid tour rows found. Check that columns are: Date, Time, Event, # TGs, Notes."
...
<div class="page">
  ...
  <div class="pane active" id="pane-upload">
    ...
    <div class="section" style="margin-top:4px;">
      <div class="rule">Step 4 &mdash; One-off tours spreadsheet (optional)</div>
      <p style="font-size:11px;font-family:system-ui,sans-serif;color:#aaa;margin-bottom:10px;line-height:1.6;">
        If you have a separate spreadsheet of one-off tours, paste or drop it here. Expected columns (tab- or comma-separated): <br/>
        <span style="font-family:monospace;font-size:11px;color:#888;">Date &nbsp;&nbsp; Time &nbsp;&nbsp; Event &nbsp;&nbsp; # TGs &nbsp;&nbsp; Notes</span>
      </p>
      <div class="drop-zone" id="dz-oneoff">
        <i class="ti ti-clipboard-list" aria-hidden="true"></i>
        <div class="dz-name" id="dz-oneoff-label">Drop spreadsheet here, or paste below</div>
        <div class="dz-hint">Tab-separated or CSV, with header row</div>
      </div>
      <input type="file" id="fi-oneoff" accept=".csv,.tsv,.txt" style="display:none;" />
      <textarea id="oneoff-paste" placeholder="Date&#9;Time&#9;Event&#9;# TGs&#9;Notes&#10;June 23&#9;1:45 PM&#9;Rebel Quest&#9;2&#9;&#10;July 7&#9;2:45 PM&#9;Camp Weequahic&#9;4&#9;" style="width:100%;min-height:80px;font-size:12px;font-family:monospace;border:1px solid #ccc;border-radius:2px;padding:8px 10px;background:#fff;resize:vertical;outline:none;margin-top:8px;"></textarea>
      <p class="msg-ok" id="ok-oneoff" style="min-height:14px;"></p>
    </div>
...
<script>
let tours=[],guides=[],scheduleResult={},countResult={},volunteersResult={},guideFileText=null,oneoffText=null;
...
makeDZ('dz-oneoff','fi-oneoff',function(f){readFile(f,function(text){oneoffText=text;document.getElementById('dz-oneoff').classList.add('loaded');document.getElementById('dz-oneoff-label').textContent=f.name;document.getElementById('ok-oneoff').textContent='File loaded.';});});
document.getElementById('fi-oneoff').addEventListener('change',function(){if(this.files[0])readFile(this.files[0],function(text){oneoffText=text;document.getElementById('ok-oneoff').textContent='Loaded.';});});
document.getElementById('oneoff-paste').addEventListener('input',function(){oneoffText=this.value.trim()||null;if(oneoffText)document.getElementById('ok-oneoff').textContent='Pasted data ready.';});
...
function proceedToReview(){
  if(!guides.length){alert('Upload a guide CSV first (step 3).');return;}
  var doReview=function(){renderReview();showPane('review');};
  var pasteText=document.getElementById('oneoff-paste').value.trim();
  var ooText=pasteText||oneoffText||null;
  if(!tours.length&&!ooText){alert('Upload at least one tour source (step 2 or step 4).');return;}
  if(ooText){
    fetch('/parse_oneoff',{method:'POST',headers:{'Content-Type':'text/plain'},body:ooText})
    .then(function(r){return r.json();}).then(function(data){
      if(data.error){alert('One-off parse error: '+data.error);return;}
      var ids=new Set(tours.map(function(t){return t.id;}));
      data.tours.forEach(function(t){if(!ids.has(t.id))tours.push(t);});
      doReview();
    });
  } else {
    doReview();
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/parse_tours":
            ...
            try:
                tours_out, err = parse_oneoff_csv(body.decode("utf-8"))
                respond({"error": err, "tours": tours_out} if err else {"tours": tours_out})
            except Exception as e:
                respond({"error": str(e), "tours": []})
