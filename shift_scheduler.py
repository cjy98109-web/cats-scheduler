...
def extract_tours_from_guides(raw_text):
    """
    Parse the guide CSV and return a pre-filled tour template CSV string
    with weekly slots and special tours extracted from the form responses.
    """
    reader = csv.reader(io.StringIO(raw_text))
    rows   = list(reader)
    if len(rows) < 2:
    ...
    <div class="two-up">
      ...
      <div class="section">
        ...
        <div id="extract-btn-row" style="display:none;margin-top:12px;">
          ...
          <p style="font-size:11px;font-family:system-ui,sans-serif;color:#aaa;margin-top:6px;line-height:1.6;">Downloads a tour_template_prefilled.csv with all weekly slots and special tours from the guide form. Open it, adjust Min/Max Guides, then upload as your tour CSV.</p>
        </div>
      </div>
    </div>
    <div class="field-row">
...
function downloadExtractedTemplate(){
  ...
  fetch('/extract_tours',{method:'POST',headers:{'Content-Type':'text/plain'},body:guideFileText})
  .then(function(r){return r.json();})
  .then(function(data){
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/parse_tours":
            ...
            try:
                template, err = extract_tours_from_guides(body.decode("utf-8"))
